...
export const getTask = async (task_id, user_id) => {
  const [result] = await pool.query(
    `
  ...
  if (status_id === old_status_id) {
    await pool.query(
      `
        ...
        WHERE order_id > (SELECT old_order_id FROM old_order) AND status_id = ? AND user_id = ?;

        UPDATE tasks
        SET order_id = order_id + 1
        WHERE order_id >= ? AND task_id != ? AND status_id = ? AND user_id = ?;

        UPDATE tasks
        SET order_id = ?
        WHERE task_id = ? AND status_id = ? AND user_id = ?;     
      `,
      [
        task_id,
        user_id,
    ...
    await pool.query(
      `
        UPDATE tasks
        SET order_id = order_id + 1
        WHERE order_id >= ? AND status_id = ? AND user_id = ?;
...
export const deleteTask = async (user_id, task_id, status_id) => {
  await pool.query(
    `
      DELETE FROM tasks
      WHERE task_id = ? AND user_id = ?;
      ...
      ORDER BY order_id;
    `,
    [task_id, user_id, status_id, user_id]
  );
};
